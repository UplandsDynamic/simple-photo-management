...
class Paginate extends React.Component {
    ...
    componentWillMount() {
        this.setState({
            ...
            totalPages: Math.ceil(this.props.record.data.count / this.props.record.meta.limit),
            currentPage: this.props.record.meta.page
        });
    }
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({
            record,
            totalPages: Math.ceil(count / record.meta.limit),
            currentPage: record.meta.page
        });
    }
    ...
    switchPage = ({linkedPage = 0, dir = 'selected'} = {}) => {
        ...
        switch (dir) {
            ...
            case 'previous':
                ...
                break;
            case 'next':
                page = this.state.totalPages > this.state.record.meta.page ?
                    this.state.record.meta.page + 1 : this.state.record.meta.page;
                url = next ? next : null;
                break;
            default:
                page = linkedPage;
