...
describe('App Component', () => {
    ...
    it('passes all props to DataTable component', () => {
        ...
        expect(dataTableWrapper.props().recordData).toEqual(instance.state.recordData);
        expect(dataTableWrapper.props().recordMeta).toEqual(instance.state.recordMeta);
        expect(dataTableWrapper.props().apiOptions).toEqual(instance.apiOptions);
        expect(dataTableWrapper.props().setRecordState).toBe(instance.setRecordState);
...
describe('App Component', () => {
    it('method setRecordState successfully changes state', () => {
        const wrapper = shallow(<App/>);
        const instance = wrapper.instance();
        instance.setRecordState({newRecord: {meta: {page: 11}, data: {results: [{user_is_admin: true}]}}});
        expect(instance.state.record.meta.page).toEqual(11)
    });
    it('method setRecordState successfully changes userIsAdmin', () => {
        const wrapper = shallow(<App/>);
        const instance = wrapper.instance();
        instance.setRecordState({newRecord: {meta: {page: 1}, data: {results: [{user_is_admin: true}]}}});
