...
class ProcessImages:
        ...
        if isinstance(directories, set):
                ...
                try:
                    if recursive:
                        if allowed_formats:
                            ...
                            item_list = (list(Path(directory).glob(match_pattern)))
                            file_urls.extend(
            ...
            for f in file_urls:
                if containing_str: # if containing_str set, filenames not containing that substring are ignored
                    if containing_str in f: yield f
                else:
                    yield f
        return False
    ...
    def add_tags(origin_file_url:str, tags:dict) -> bool:
        ...
        try:
            ...
            tags_to_write = ProcessImages.read_iptc_tags(filename=origin_filename, path=path)
            # merge existing & new tags to one list
            if tags_to_write:
